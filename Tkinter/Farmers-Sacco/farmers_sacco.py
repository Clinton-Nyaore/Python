...
my_db = mysql.connector.connect(host="localhost", user="root", passwd="", database="register")
...
class SignUp:
    def __init__(self, root):
        self.window = root
        self.window.title("Wakulima Farmers Sacco")
        ...
        title2 = Label(frame, text="Join with us Wakulima Farmers Sacco", font=("times new roman", 13), bg="white", fg="red").place(x=20,
                                                                                                              y=50)
        ...
        password = Label(frame, text="New password", font=("helvetica", 15, "bold"), bg="white").place(x=20, y=290)

        self.password_txt = Entry(frame, font=("arial"))
        self.password_txt.place(x=20, y=320, width=320)
    ...
    def signup_func(self):
        if self.fname_txt.get() == "" or self.lname_txt.get() == "" or self.email_txt.get() == "" or self.password_txt.get() == "":
            ...
            try:
                ...
                cur.execute("select * from farmer_register where email=%s", self.email_txt.get())
                row = cur.fetchone()
                ...
                if row != None:
                    ...
                    cur.execute(
                        "insert into farmer_register (f_name,l_name,email,password) values(%s,%s,%s,%s)",
                        (
                            self.fname_txt.get(),
                            self.lname_txt.get(),
                            self.email_txt.get(),
