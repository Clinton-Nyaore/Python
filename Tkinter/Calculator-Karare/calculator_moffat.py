...
my_db = mysql.connector.connect(host="localhost", user="root", passwd="", database="karare")
...
top.title('Moffat Calculator')
top.configure(bg='black')

l = Label(top, text='Moffat Calculator', fg='black').grid(row=0, column=1, sticky=W)
...
def proces():
    ...
    sql = "INSERT INTO calculations (what, answer) VALUES (%s, %s)"
    val = ("Answer", answer)
...
B = Button(top, text="Calculate & Save", command=proces, bg='red').grid(row=5, column=1)
